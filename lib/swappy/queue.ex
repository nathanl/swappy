defmodule Swappy.Queue do
  ...
  defmodule Manager do
    ...
    defp manage_queue(spawner_pid, results, _job, _idle_workers, result_count, limit)
         when result_count >= limit do
      send(spawner_pid, {:results, results})
    end

    # done because nothing left to do, yay!
    defp manage_queue(spawner_pid, results, [] = _jobs, idle_workers, _result_count, _limit)
         when length(idle_workers) == @worker_count do
      send(spawner_pid, {:results, results})
    end
    ...
    defp manage_queue(spawner_pid, results, jobs, idle_workers, result_count, limit) do
      receive do
        {:worker_results, new_anagrams, new_jobs, worker_pid} ->
          ...
          manage_queue(
            spawner_pid,
            results ++ new_anagrams,
            new_jobs ++ jobs,
            [worker_pid | idle_workers],
            new_result_count,
            limit
          )
      end
  ...
  defmodule Worker do
    ...
    defp do_work(jobs, found_anagrams, _completed_job, result_count, limit)
         when result_count == limit do
      {found_anagrams, jobs}
    end
